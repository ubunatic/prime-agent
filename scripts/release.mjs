 ...
 *
 * Usage:
 *   node scripts/release.mjs <major|minor|patch> [--skip-npm-publish] [--dry-run]
 *   node scripts/release.mjs <x.y.z> [--skip-npm-publish] [--dry-run]
...
const skipNpmPublish =
	process.env.PI_SKIP_NPM_PUBLISH === "1" ||
	process.env.PI_SKIP_NPM_PUBLISH === "true" ||
	rawArgs.includes("--skip-npm-publish");
const RELEASE_TARGET = rawArgs.find((arg) => !arg.startsWith("--"));

if (rawArgs.includes("--help") || rawArgs.includes("-h")) {
	console.log("Usage: node scripts/release.mjs <major|minor|patch|x.y.z> [--skip-npm-publish] [--dry-run]");
	process.exit(0);
}
...
if (!RELEASE_TARGET || (!BUMP_TYPES.has(RELEASE_TARGET) && !SEMVER_RE.test(RELEASE_TARGET))) {
	console.error("Usage: node scripts/release.mjs <major|minor|patch|x.y.z> [--skip-npm-publish] [--dry-run]");
	process.exit(1);
}
