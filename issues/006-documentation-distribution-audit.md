...
The fork documentation described GitHub Releases as the permanent and exclusive distribution architecture while the implementation still named an unavailable custom-domain default. Live release validation established GitHub Releases as the working default, with configurable custom mirrors and optional Cloudflare R2 publishing.

## Findings

1. `packages/coding-agent/src/utils/version-check.ts` and `scripts/pack-prime-agent-release.mjs` originally defaulted to `https://ubunatic.com/prime-agent`, whose release manifest returned 404 during live validation.
...
1. Update `docs/ForkArchitecture.md` to describe the GitHub Releases default, configurable mirrors, and optional R2 publishing accurately.
...
- Documentation names GitHub Releases as the runtime default and explains custom mirror configuration.
...
- Updated the architecture guide to document the GitHub Releases default, configurable mirrors, conditional R2 publishing, and both release channels.
