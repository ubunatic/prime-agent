...
Prime Agent stable builds fetch `https://github.com/ubunatic/prime-agent/releases/latest/download/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` from the `beta` release and remain on the beta channel. Override the release download base with `PRIME_AGENT_DOWNLOAD_BASE_URL`. Disable version checks with `PI_SKIP_VERSION_CHECK=1`.
